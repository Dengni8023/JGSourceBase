...
ConfigFile="${PROJECT_DIR}/${PROJECT_NAME}/${TARGET_NAME}.xcconfig"
if [[ ! -f ${ConfigFile} ]]; then
    echo "Could not found ${TARGET_NAME}.xcconfig"
    exit
fi

# 根据 JGSourceBase 文件夹下文件最后修改时间生成相关配置信息
# 文件夹的最后更新时间与文件夹下所有文件的最后一次修改时间一致
# ModifyDateTime=`stat -f "%Sm" -t "%Y%m%d.%H%M%S" "${PROJECT_DIR}"`
...
JGSBuild: ${Build}
JGSResourceDate: ${ResourceDate}
-------- Modify ${TARGET_NAME}.xcconfig --------
"

sed -i '' 's/^\(JGSBuild = \).*/\1'"${Build}"'/'  "${ConfigFile}"
sed -i '' 's/^\(JGSResourceDate = \).*/\1'"${ResourceDate}"'/'  "${ConfigFile}"
